...
namespace Glacier
{
    ...
    const RTP::ZPropertyInfo& ZCHAROBJ::GetProperties() const { return ZCHAROBJ::Info; }
    uint32_t ZCHAROBJ::GetObjectId() const { return ZCHAROBJ::m_Id; }
    void ZCHAROBJ::GetObjectIdAndMask(uint32_t& id, uint32_t& mask) const { id = ZCHAROBJ::m_Id; mask = ZCHAROBJ::m_Mask; }
    ZGEOMCLASSINFO* ZCHAROBJ::GetOldClassInfo() const { return ZCHAROBJ::m_OldClassInfo; }
    ...
    bool ZCHAROBJ::CreateGeometry()
    {
        ...
        // TODO: Finish this place after CreateWinFaces will be reversed
        // PC builds glyph faces, groups them by texture and creates one sprite primitive per group.
        return false;
    }

    void ZCHAROBJ::SetFont(ZFONT* pFont) { m_pFont = pFont; }
    ...
    int32_t ZCHAROBJ::GetCharPosition(int lIdx)
    {
        ...
        for (int i = 0; i < lIdx; ++i)
        {
            ...
            if (previous) position -= m_pFont->GetKerning(previous, ch);
            previous = ch == ' ' ? 0 : ch;
        }
        return position;
    }
    ...
    void ZCHAROBJ::SetColor(uint32_t lColor)
    {
        ...
        if ((color & 0xFFFFFF) == (m_dwFaceColor & 0xFFFFFF)) return;
        m_dwFaceColor = color;
        m_bAnimateAlpha = false;
        for (uint32_t i = 0; m_pSpriteArrays && i < m_iNumSpriteArrays; ++i)
        {
            if (m_pPrims[i] == m_iIconPrim) continue;
    ...
    void ZCHAROBJ::SetLineSpacing(int8_t spacing) { m_iLineSpacing = spacing; }
    void ZCHAROBJ::SetTabSize(int8_t size) { m_iTabSize = size; }
    int32_t ZCHAROBJ::GetTabSize(int8_t) const { return m_iTabSize; }
    void ZCHAROBJ::SetMonoSpace(int8_t size) { m_iMonoSpaceSize = size; }
    const char* ZCHAROBJ::GetText() const { return m_szTextPtr; }
    ZFONT* ZCHAROBJ::GetFont() const { return m_pFont; }
    void ZCHAROBJ::SetSpacingAdd(int8_t spacing) { m_iSpacingAdd = spacing; }
    void ZCHAROBJ::GetFontRef(ZGEOMREF&) { ZASSERT(false); }
    void ZCHAROBJ::SetFontRef(const ZGEOMREF& font) { SetFont(geom_cast<ZFONT>(ZGEOM::RefToPtr(font.m_Value))); }
    void ZCHAROBJ::GetColor(float (&)[3]) { ZASSERT(false); }
