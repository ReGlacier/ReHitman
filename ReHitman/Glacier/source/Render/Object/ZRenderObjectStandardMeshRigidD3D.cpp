...
namespace Glacier
{
    ZRenderObjectStandardMeshRigidD3D::ZRenderObjectStandardMeshRigidD3D(const ZPrimHandle& hPrim, ZRenderMaterialInstance* pMaterialInstance)
        ...
        , m_bVerticesUploaded(false)
    {
        ...
        m_VertexContainer.Create(pSubMesh->lNumVertices, sizeof(SVertexWintelDP3), ZSharedResourcesD3D::g_pInstance->m_pVertexAllocator, 0u);
    ...
    ZRenderObjectStandardMeshRigidD3D::~ZRenderObjectStandardMeshRigidD3D()
    {
        m_VertexContainer.Release();
        m_IndexContainer.Release();
    }

    ZRenderObjectInstance* ZRenderObjectStandardMeshRigidD3D::CreateInstance(ZBaseGeom* pBaseGeom)
    {
        return ZUniMemory::New<ZRenderObjectInstanceStandardMeshRigidD3D>(this, pBaseGeom);
    }

    void ZRenderObjectStandardMeshRigidD3D::Update()
    {
        ...
        if (pBuffer)
        {
            memcpy(pBuffer + m_VertexContainer.m_lVertexOffset, pVertices, m_VertexContainer.m_lNumVertices * sizeof(SVertexWintelDP3));
