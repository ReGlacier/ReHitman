...
namespace Glacier
{
    ZRenderObjectStandardMeshWeightedD3D::ZRenderObjectStandardMeshWeightedD3D(const ZPrimHandle& hPrim, ZRenderMaterialInstance* pMaterialInstance)
        ...
        , m_bVerticesUploaded(false)
    {
        ...
        m_VertexContainer.Create(pSubMesh->lNumVertices, sizeof(SVertexW4WintelDP3), ZSharedResourcesD3D::g_pInstance->m_pVertexAllocator, 0u);
    ...
    ZRenderObjectStandardMeshWeightedD3D::~ZRenderObjectStandardMeshWeightedD3D()
    {
        m_VertexContainer.Release();
        m_IndexContainer.Release();
    }

    ZRenderObjectInstance* ZRenderObjectStandardMeshWeightedD3D::CreateInstance(ZBaseGeom* pBaseGeom)
    {
        return ZUniMemory::New<ZRenderObjectInstanceStandardMeshWeightedD3D>(this, pBaseGeom);
    }

    void ZRenderObjectStandardMeshWeightedD3D::Update()
    {
        ...
        const SVertexW4WintelDP3* pVertices = ZPrimHandle{m_lVertices};
        auto* pBuffer = static_cast<uint8_t*>(ZSharedResourcesD3D::g_pInstance->m_pSVB->LockedData());
        if (pBuffer)
        {
            memcpy(pBuffer + m_VertexContainer.m_lVertexOffset, pVertices, m_VertexContainer.m_lNumVertices * sizeof(SVertexW4WintelDP3));
