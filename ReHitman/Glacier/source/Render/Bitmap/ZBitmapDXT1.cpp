...
namespace Glacier
{
    ...
    ZBitmapDXT1::~ZBitmapDXT1() = default;

    void ZBitmapDXT1::GetRGBA(uint8_t* pDst, int px, int py, int dx, int dy, int pitch, int dwLevel) const
    {
        // PC decodes the whole mip level into pDst; the sub-rect and pitch
        // arguments are ignored.
        DecodeMipLevel(reinterpret_cast<int*>(pDst), dwLevel);
    }

    void ZBitmapDXT1::GetData(ZBitmap::TDataBlock* pData, int dwMipLevel)
    {
    ...
    void ZBitmapDXT1::DecodeMipLevel(int* pDst, int dwLevel) const
    {
        const TMipLevel* pMipLevel = const_cast<ZBitmapDXT1*>(this)->GetMIPLevel(dwLevel);
        ...
        for (int by = 0; by < lBlockRows; ++by)
        {
            if (lSizeX - 3 > 0)
            {
                ...
                {
                    Decode4x4DXT_RGB(pOutBlock, reinterpret_cast<const uint16_t*>(pSrc), lSizeX);
                    pSrc += 8;
                    pOutBlock += 4;
                }
                pBlock += 8 * lBlockCols;
    ...
    void ZBitmapDXT1::PackMipLevel(int level, int sx, int sy, const uint32_t* pRGBA)
    {
        const int lSizeX = (sx + 3) & ~3;
        const int lSizeY = (sy + 3) & ~3;
        const int lSize = (lSizeX * lSizeY) >> 3; // 8 bytes per 4x4 block
                ...
                if (lSizeX - 3 > 0)
                {
                    ...
                    do
                    {
                        ...
                        for (int row = 0; row < 4; ++row)
                        {
                            ...
                            {
                                const int gx = lRemX <= col ? sx - 1 : x0 + col;
                                const uint32_t px = pRGBA[gx + sx * gy];
                                block[4 * row + col] = px;
                                if ((px & 0xFF000000) < 0x80000000)
                                {
                                    bAlpha = 1;
                                }
                            }
                        }
                        CompressBlockDXT1(bAlpha, 4, 0, 0, block, scratch, out);
                        pBuffer[lOutIndex] = out[0];
                        pBuffer[lOutIndex + 1] = out[1];
                        x0 += 4;
                        lOutIndex += 2;
                        lRemX -= 4;
                    }
