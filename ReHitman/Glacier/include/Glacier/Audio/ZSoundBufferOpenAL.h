...
namespace Glacier
{
    class ZSoundBufferOpenAL : public ZSoundBufferWintel
    {
    public:
        explicit ZSoundBufferOpenAL(ZSynth* _synth);
        ~ZSoundBufferOpenAL() override = default;
        ...
        void Pause() override;
        int GetNumStreamSamples() override;
        ...
        uint32_t m_lALSource;
        uint32_t m_lALBuffers[2];
    };

    RE_VERIFY_OFFSET(ZSoundBufferOpenAL, m_lALSource, 0x63F4);
    RE_VERIFY_OFFSET(ZSoundBufferOpenAL, m_lALBuffers, 0x63F8);
    RE_VERIFY_SIZE(ZSoundBufferOpenAL, 0x6400);
}
